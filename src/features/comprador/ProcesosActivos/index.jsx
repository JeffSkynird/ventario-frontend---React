...
export default function index() {
  ...
  const { isLoading, isError, data, error , refetch,} = useQuery(['getResults',usuario.token,'vendedor',usuario.user.companyId], obtenerTodos)
    ...
    Cell: (value) => (
      <div style={{display:'flex'}}>
        <IconButton aria-label="delete" onClick={() => {
          navigate('/activos/'+value.row.original.id, { state:{item: value.row.original,isNew:false}})
