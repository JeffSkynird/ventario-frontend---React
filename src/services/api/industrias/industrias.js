...
export const  obtenerTodos = async ({queryKey}) => {
    ...
    let url = import.meta.env.VITE_API_URL+ "industries"
    let setting = {
        method: "GET",
        url: url,
...
export const editar = async (obj,token) => {
    let url = import.meta.env.VITE_API_URL+ "industries/"+obj.id
    let setting = {
        method: "PUT",
        url: url,
        data: obj,
...
export const eliminar = async (id,token) => {
    let url = import.meta.env.VITE_API_URL+ "industries/"+id
    let setting = {
        method: "DELETE",
        url: url,
        headers: { 'Accept': 'application/json','Authorization': `Bearer ${token}` }
